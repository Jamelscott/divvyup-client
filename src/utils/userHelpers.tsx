...
export async function loginEmailOrUsername(
  ...
  if (usernameOrEmail && password) {
    ...
    if (isEmail) {
      ...
      if (response.error) {
        notifications.show(
          errorNotification("Failed to login", "login info is incorrect")
        );
        return undefined;
      }
      ...
      const { data, error } = await supabase
        .from("profiles")
        .select()
        .eq("username", usernameOrEmail.toLocaleLowerCase());
...
export function userBuilder(user: User): User {
  ...
  if (user.email === undefined)
    throw new Error("User email data is undefined?");
...
export const handleUpdateUserExpenseSession = (newValue: ExpenseData): User => {
  ...
  const updatedUser = {
    ...parsedUser,
    expenses: [newValue, ...parsedUser.expenses],
  };
...
export const handleDeleteUserExpenseSession = (expense: ExpenseData): User => {
  ...
  const updatedUser = {
    ...parsedUser,
    expenses: [...existingExpenses.filter((exp) => exp.id !== expense.id)],
  };
  sessionStorage.setItem("user", JSON.stringify(updatedUser));
  return updatedUser;
};
...
export const handleSignUpSubmit = async (
  ...
  if (!email || email.split("@")[0].length < 4) {
    notifications.show(
      errorNotification("invalid email", "input a valid email")
    );
    return false;
  }
  ...
  if (password !== confirmPassword) {
    notifications.show(
      errorNotification("invalid confirmation", "passwords do not match")
    );
    return false;
  }
  const { data, error } = await supabase.auth.signUp({
  ...
  if (error) {
    if (
      ...
      notifications.show(
        errorNotification("invalid username", "that username already exists")
      );
      return false;
    } else {
  ...
  if (data.user === null) {
    notifications.show(
      errorNotification(
        "login error",
        "user not found, returning null from server"
      )
    );
    return false;
  }
  if (data.user.email === undefined) {
    notifications.show(
      errorNotification(
        "login error",
        "user email not found, returning null from server"
      )
    );
    return false;
  }
