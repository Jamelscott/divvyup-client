...
import Expense from "@/InProgress/Friend/components/Expense";
    ...
    <div className="wrapper">
      ...
      >
        {sharedExpenses.map((expense) => (
          <Expense expense={expense} sourceType="userSpecfic" />
        ))}
      </div>
    </div>
  );
    ...
    >
      <NeonGradientCard className="max-w-sm items-center justify-center text-center">
        {[...friends, ...friends].map((friend, index) => {
          ...
          const sharedExpenses = user.expenses.filter(
            (expense) => expense.lender === user.id || expense.ower === user.id
          );
