...
function ExpenseList({
  ...
  const friendUsername = () => {
    ...
    const id =
      friendExpenses[0].lender === user.id
        ? friendExpenses[0].ower
        : friendExpenses[0].lender;
  ...
  const header =
    sourceType === ExpenseListType.RECENT ? (
      ...
      <h3>
        <b>Expenses with {friendUsername()}</b>
      </h3>
    );
  return (
    <div
      className="flex flex-col gap-5 items-center w-full bg-white shadow-lg rounded-xl border border-black p-3"
      style={{ height: "100%" }}
    >
      {header}
      <div
        className="flex p-3 flex-col gap-5 w-full h-full overflow-y-scroll"
        style={{ height: "fit-content" }}
      >
        {renderedExpenses.length > 0 ? (
          renderedExpenses.map((expense: ExpenseData, i) => {
            return (
              <Expense
                key={`${expense} ${i}`}
                expense={expense}
