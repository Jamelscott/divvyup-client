...
function Expense({ expense, sourceType }: { expense: ExpenseData, sourceType: ExpenseListType }) {
    ...
    }).image(friend?.photo).resize(fill().width(40).height(40)).roundCorners(byRadius(100));
        ...
        <>
            <div className="flex bg-white border-2 border-grey rounded-3xl p-3 shadow-xl justify-between w-full h-22" key={`${expense.id}`}>
                <div className="flex gap-10">
                    ...
                    {
                        sourceType === ExpenseListType.USER_SPECIFIC && <div className="flex flex-col items-center justify-center w-12">
                            <img src={`${expenseIcon}`} alt="" />
                            <p style={{ margin: '0 0' }}>{expense.type}</p>
                        </div>
                    }
                </div >
                <div className="flex flex-col w-fit items-center justify-center w-20 text-center text-lg font-bold leading-6">
                    <p style={{ margin: '0 0' }}>{expense.name}</p>
                </div>
                <div className="flex gap-10 w-fit">
                    {sourceType === ExpenseListType.USER_SPECIFIC && <>
                        {!youOwe ? <div className="flex flex-col justify-center text-nowrap">
                            <p style={{ margin: '0 0' }}>You paid</p>
                            <div className="text-center">
                                <p style={{ margin: '0 0' }}>{`$${expense.quantity}`}</p>
                            </div>
                        </div> : <div className="flex flex-col items-end justify-center text-nowrap">
                    ...
                    <div className="flex flex-col items-end justify-center text-nowrap w-fit">
                        <p style={{ margin: '0 0' }}>{!youOwe ? `${friend?.username} owes you` : `You owe ${friend?.username}`}</p>
                        <p><span style={{ color: `${!youOwe ? 'green' : 'red'}` }}>${priceParser(expense)}</span> ({expense.splitpercentage}%)</p>
                    </div>
                    {sourceType === ExpenseListType.USER_SPECIFIC && <div className="flex flex-col justify-between items-center">
                        <Icons.edit handleOnClick={() => setOpenEditModal(true)} />
                        <Icons.delete handleOnClick={() => setOpenConfirmModal((prev) => !prev)} />
                    </div>}
                </div>
            </div >
            {openEditModal && <EditExpense setOpenEditModal={setOpenEditModal} expenseData={expense} friendData={friend as User} />}
            {openConfirmModal && <ConfirmationModal onClickNo={() => setOpenConfirmModal(false)} onClickYes={() => handleDeleteExpense()} message={'Are you sure you want to delete this expense?'} />}
        </>
    );
}
    ...
    edit: ({ handleOnClick }: any) => (
        <div onClick={() => handleOnClick()}>
            <svg width="14" height="14" viewBox="0 0 14 14" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path d="M12.0505 1.94979C11.5459 1.44521 10.8751 1.16693 10.1616 1.16693C9.44823 1.16693 8.77741 1.44521 8.27283 1.94979L2.12857 8.09402C1.86898 8.3536 1.69981 8.68205 1.63973 9.04371L1.17891 11.8075C1.13224 12.0875 1.22442 12.3751 1.4245 12.5752C1.59075 12.7414 1.81648 12.833 2.04807 12.833C2.09648 12.833 2.14431 12.8289 2.19273 12.8208L4.95658 12.3599C5.31825 12.2998 5.64666 12.1307 5.90624 11.8711L12.0505 5.72686C12.5551 5.22227 12.8333 4.55084 12.8333 3.838C12.8333 3.12459 12.5551 2.45437 12.0505 1.94979ZM5.08141 11.0474C4.99507 11.1338 4.88541 11.1904 4.76525 11.2102L2.39574 11.6051L2.79066 9.23562C2.81049 9.11545 2.86707 9.00579 2.9534 8.91945L7.56524 4.30762L9.69324 6.43559L5.08141 11.0474ZM11.2257 4.90263L10.5181 5.61015L8.39007 3.48218L9.09766 2.77459C9.38175 2.4905 9.76032 2.3336 10.1616 2.3336C10.5636 2.3336 10.9416 2.4905 11.2257 2.77459C11.5098 3.05867 11.6667 3.43666 11.6667 3.83857C11.6667 4.24049 11.5098 4.61855 11.2257 4.90263Z" fill="#41416E" />
            </svg>
        </div>
    ),
